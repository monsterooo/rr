...
var CanvasHelp = require('./CanvasHelp');
...
function drawImageRenderLayer (ctx, layer) {
  ...
  CanvasHelp.drawImage(ctx, image, layer.frame.x, layer.frame.y, layer.frame.width, layer.frame.height);
}
...
function drawTextRenderLayer (ctx, layer) {
  ...
  if (!Font.isFontLoaded(fontFace)) {
    return;
  }

  CanvasHelp.drawText(ctx, layer.text, layer.frame.x, layer.frame.y, layer.frame.width, layer.frame.height, fontFace, {
...
function drawGradientRenderLayer (ctx, layer) {
  ...
  CanvasHelp.drawGradient(ctx, x1, y1, x2, y2, layer.colorStops, layer.frame.x, layer.frame.y, layer.frame.width, layer.frame.height);
}
